...
local function push_to_remote(remote, branch, force, callback)
	...
	vim.system(args, { text = true }, function(obj)
		vim.schedule(function()
			if obj.code == 0 then
				callback(true, "Pushed to " .. remote .. "/" .. branch .. (force and " (forced)" or ""))
			else
...
local function pull_and_push(remote, branch, force, callback)
	vim.system({ "git", "pull", remote, branch }, { text = true }, function(pull_obj)
		if pull_obj.code ~= 0 then
			vim.schedule(function()
				callback(false, "Pull failed: " .. pull_obj.stderr)
			end)
			return
		end
		push_to_remote(remote, branch, force, callback)
	end)
end
...
local function commit_and_push(branch, message, callback)
	commit_to_branch(branch, message, function(success, output)
		...
		get_current_branch(function(current_branch)
			push_to_remote("origin", current_branch, false, function(push_success, push_output)
				if push_success then
					callback(true, output .. "\n" .. push_output)
				else
					vim.notify(push_output .. "\nChoose: Pull & Push or Force", vim.log.levels.WARN)
					require("snacks").picker({
						title = "Push Conflict",
						items = {
							{ text = "Pull & Push", icon = "󰜐", action = "pull" },
							{ text = "Force Push", icon = "󰜤", action = "force" },
						},
						format = function(i, _)
							return {
								{ i.icon, "SnacksPickerIcon" },
								{ " " },
								{ i.text, "SnacksPickerTitle" },
							}
						end,
						confirm = function(self, action_item)
							self:close()
							local push_fn = action_item.action == "pull" and pull_and_push or push_to_remote
							local force = action_item.action == "force"
							push_fn("origin", current_branch, force, function(s, out)
								callback(s, output .. "\n" .. out)
							end)
						end,
					})
		...
		get_status(function(status)
			...
			get_remotes(function(remotes, err)
				...
				confirm = function(self, item)
					if item then
						self:close()
						vim.system({ "git", "fetch", item.remote }, { text = true }, function()
							vim.system({ "git", "merge", item.remote .. "/" .. item.branch }, { text = true }, function(merge_obj)
								if merge_obj.code == 0 then
									push_to_remote(item.remote, item.branch, false, function(success, output)
										if success then
											vim.notify(output, vim.log.levels.INFO)
										else
											vim.notify(output .. "\nUse 'Pull & Push' or 'Force Push'", vim.log.levels.ERROR)
										end
									end)
								else
									require("snacks").picker({
										title = "Push Conflict - Choose Action",
										items = {
											{ text = "Pull & Push", icon = "󰜐", action = "pull" },
											{ text = "Force Push", icon = "󰜤", action = "force" },
											{ text = "Cancel", icon = "󰅖", action = "cancel" },
										},
										format = function(i, _)
											return {
												{ i.icon, "SnacksPickerIcon" },
										...
										confirm = function(self2, action_item)
											self2:close()
											if action_item.action == "pull" then
												pull_and_push(item.remote, item.branch, false, function(s, out)
													if s then vim.notify(out, vim.log.levels.INFO)
													else vim.notify(out, vim.log.levels.ERROR) end
												end)
											elseif action_item.action == "force" then
												push_to_remote(item.remote, item.branch, true, function(s, out)
													if s then vim.notify(out, vim.log.levels.INFO)
													else vim.notify(out, vim.log.levels.ERROR) end
												end)
											end
